...
option_list = list(
    make_option(c('-i', '--input_blast'),
                default = input_exon_table_def,
...
input_blast_file = opt$input_blast
...
if (use_hmpfix == 1) {
   if ('read_seq_corrected' %in% colnames(blast.dt) &
      ...
      print('  - using homopolymer corrected sequences')
   } else {
      print('  ? homopolymer corrected columns not found. Using uncorrected sequences.')
   }
}
...
# exon_comb_sum.dt[, exon_comb_short := as.character(
#    lapply(
#       exon_comb_dir, 
#       function (x) comb_text_short(as.integer(strsplit(x, ',')[[1]]))
#    )
# )]
# exon_comb_sum.dt[, exon_comb_short := factor(
#    exon_comb_short, levels=rev(exon_comb_short))]
# exon_comb_sum.dt[, exon_comb_dir := factor(
#    exon_comb_dir, level=rev(exon_comb_dir))]

# Plot only specific combinations, extract these here
# exon_combinations_long = exon_comb_sum.dt[exon_comb_dir_count>8, exon_comb_dir]
# exon_combinations_short = exon_comb_sum.dt[exon_comb_dir_count>8, exon_comb_short]

# Make a new folder for each exon combination
cat('OK.\n')


#exon_comb_len.dt = unique(blast.dt[, .(exon_no, exon_length, exon_comb_order, exon_comb_short)])[, list(exon_length = sum(exon_length)), by=c('exon_comb_short')]
#
# Generate a new data frame with all exon-exon join pairs
#
# How many nucleotides are missing from the start of each exon from the alignment?
...
# write.table(x=exon_joins.dt[, .(exon_join, read_id, exon_left_read_end_pos_align, exon_right_read_start_pos_align, 
#                                 exon_comb_short_rc, read_count)],
#             file=paste0(output_data, input_base, '_exon_joins_read_positions.txt'),
